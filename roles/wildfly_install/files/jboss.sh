...
#Herausfinden der Major-Version über die Variable $JBOSS_RELEASE_NAME
if [ ! X$JBOSS_RELEASE_NAME = X ]; then
 	product=`echo $JBOSS_RELEASE_NAME | perl -ne 'print /(.*?)-\d+/'`
 	major=`  echo $JBOSS_RELEASE_NAME | perl -ne 'print /.*?-(\d+)/'`
	case "$product" in
	  *eap)
		case "$major" in
	  	  6)
...
admin() {
  if [ "x$OPENSHIFT_HOMEDIR" = "x" ]; then
      echo "$JBOSS_HOME/bin/jboss-cli.sh --connect --controller=${BIND_ADDRSS-localhost}:$JBOSS_ADMIN_PORT" "$@"
      $JBOSS_HOME/bin/jboss-cli.sh --connect --controller=${BIND_ADDRSS-localhost}:$JBOSS_ADMIN_PORT "$@"
  else
    thisdir=${PWD}
    cd $JBOSS_HOME/bin/
...
case "$action" in
 ...
 #   $start > $out 2>&1 &
    rm -rf $rundir/tmp
    $JBOSS_HOME/bin/standalone.sh -b 0.0.0.0 -bmanagement 0.0.0.0 -Djboss.node.name=$JBOSS_INSTANZ_NAME $OFFSET_PROP $JBOSS_OPTS > $out 2>&1 &
    cnt=$START_TIMEOUT
    sleep 1
