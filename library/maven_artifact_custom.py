...
options:
    ...
    local:
        description:
            - If C(yes), copy artifact from the maven local repository.
            - If C(yes), the default I(repository_url) is 'file:///{user.home}/.m2/repository'
            - I(repository_url) must be defined with the file scheme 'file:///'
        required: false
        default: 'no'
        choices: ['yes', 'no']
        version_added: "2.6"
...
class MavenDownloader:
    ...
    def find_latest_version_available(self, artifact):
        ...
        content = self._getContent(self.base + path, "Failed to download the maven metadata file: " + path)
    ...
    def download(self, artifact, filename=None):
        ...
        if not self.verify_md5(filename, url):
            parsed_url = urlparse(url)
            if self.local:
                if os.path.isfile(parsed_url.path):
    ...
    def verify_md5(self, file, remote_url):
        if os.path.exists(file):
            ...
            if self.local:
                ...
                remote_md5 = self._getContent(remote_url + '.md5', "Failed to download MD5", False)
...
def main():
    ...
    if not repository_url:
        ...
        if local:
            repository_url = "file://" + os.path.expanduser("~/.m2/repository")
    try:
        parsed_url = urlparse(repository_url)
    except AttributeError as e:
        module.fail_json(msg='url parsing went wrong %s' % e)

    if local and parsed_url.scheme != "file":
        module.fail_json(msg="for local maven repo, you must use the 'file' scheme in the url instead of: %s" % parsed_url.scheme)
    ...
    if os.path.lexists(b_dest) and downloader.verify_md5(dest, downloader.find_uri_for_artifact(artifact)):
        prev_state = "present"
